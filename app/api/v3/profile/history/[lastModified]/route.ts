...
export async function GET(req: NextRequest, ctx: { params: Promise<{ lastModified: string }> }) {
  ...
  try {
    const profiles = await db.getProfiles({ dateFrom: since, count: limit });
    const maxTs    = maxTimestamp(profiles.map((p) => p.created_at));
    const lm       = new Date(maxTs).toUTCString();
    return withCors(
      NextResponse.json({ status: 200, result: profiles }, {
